...
class PeriodicMLLMCheckpoint(pl.Callback):
    ...
    def __init__(self, save_dir: str, every_n_steps: int = 100, filename_prefix: str = "mllm"):
        super().__init__()
    ...
    def on_train_batch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, outputs, batch, batch_idx: int) -> None:
        ...
        if global_step > 0 and global_step % self.every_n_steps == 0:
            self._save_checkpoint(trainer, pl_module, tag="step")
    ...
    def _save_checkpoint(self, trainer: pl.Trainer, pl_module: pl.LightningModule, tag: str) -> None:
        ...
        filename = f"{self.filename_prefix}-{tag}-step{step_num:06d}-{timestamp}.pt"
